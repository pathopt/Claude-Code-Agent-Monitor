...
### Compatibilidad con Cursor

Cursor no necesita una opción de configuración separada: elegir **Claude Code** en la pantalla inicial también habilita Cursor. Un observador del sistema de archivos detecta `~/.cursor/chats/*/<session>/meta.json` en cuanto se inicia `agent`, antes de que exista la transcripción; los cambios de `prompt_history.json` actualizan de inmediato la tarjeta, el estado de trabajo y Conversation al enviar un prompt. Cuando aparece `~/.cursor/projects/*/agent-transcripts`, el panel integra el turno pendiente sin duplicarlo, completa títulos/proyectos/turnos/subagentes y conserva los JSONL principales y secundarios.

Las tarjetas muestran `Cursor · <título>` y subtítulos de proyecto, turnos y subagentes. La tabla editable **Precios de Cursor** cubre Grok, Composer y el catálogo publicado de terceros sin reutilizar tarifas de Claude o Codex. `DASHBOARD_CURSOR_HOME` ajusta la ruta y `DASHBOARD_CURSOR_SYNC_MS` el sondeo de respaldo; `0` desactiva solo el escaneo periódico y mantiene activo el observador en tiempo real.
...
```mermaid
flowchart LR
    A["El usuario selecciona un idioma<br/>en / zh / vi / ko / es"] --> B["Detector i18next<br/>localStorage -> navigator"]
    B --> C["Idioma resuelto<br/>alternativa: en"]
    C --> D["Recursos por espacio de nombres<br/>common/nav/dashboard/..."]
    D --> E["Hooks React useTranslation"]
    E --> F["IU y etiquetas de accesibilidad traducidas"]
```
...
> **Cursor viene integrado:** CCAM descubre el historial nativo de Cursor en `~/.cursor/projects/*/agent-transcripts`, lo enriquece con metadatos de `~/.cursor/chats`, lo guarda como proveedor `cursor` independiente y conserva instantáneas de las conversaciones antes de que Cursor limpie los archivos. Cursor se incluye automáticamente al elegir el ámbito de Claude Code. Las fuentes SSH remotas solo reflejan actualmente los directorios de Claude Code y Codex; para importar Cursor remoto, monta o expón su directorio localmente y configura `DASHBOARD_CURSOR_HOME`.
