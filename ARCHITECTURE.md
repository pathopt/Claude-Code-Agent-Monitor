...
Agent Dashboard is a local-first monitoring platform for Claude Code, Cursor, and Codex sessions. It combines native lifecycle hooks with provider-aware transcript discovery, persists them in SQLite, and presents them through a React dashboard with real-time WebSocket updates.

> **Cursor is a first-class stored provider.** `server/index.js` watches both `~/.cursor/chats` and `~/.cursor/projects`: `meta.json` creates the stored session at CLI launch, `prompt_history.json` updates cards and Conversation at prompt submission and persists a deduplicated `cursor_user_message` activity row, and the later canonical transcript is merged by stable message identity. `server/lib/cursor-ingest.js` also runs a fingerprinted polling fallback, repairs older hook-created placeholders, imports subagents, and snapshots JSONL under `<dashboard-data>/cursor-transcripts`. Conversation routes resolve the live file first and the snapshot second while falling back to prompt history before either exists. The product selector intentionally groups Cursor under the Claude Code scope, while storage and pricing retain `provider = 'cursor'` so billing never crosses rate cards.
...
- Cursor history and conversation retention -- automatic backfill plus durable snapshots
...
| `server/lib/security.js`  | Network-hardening module (fix for GHSA-gr74-4xfh-6jw9). `resolveHost()` picks the bind address. `hostGuard` rejects requests whose `Host` is not loopback, operator-allowlisted, or the Kubernetes `POD_IP`. `DASHBOARD_TOKEN` / `DASHBOARD_TOKEN_FILE` protects REST and WebSocket access. Independent `DASHBOARD_HOOK_TOKEN` / `DASHBOARD_HOOK_TOKEN_FILE` protects the loopback hook routes (`/api/hooks/event`, `/api/hooks/codex`) when remote ingestion is enabled. A third, deliberately separate `REMOTE_PUSH_TOKEN` / `REMOTE_PUSH_TOKEN_FILE` gates `POST /api/hooks/ingest-batch` — the only route meant to be reachable from the public internet — so hardening the loopback routes never opens the internet-writable one as a side effect; unset, that route answers `503 REMOTE_PUSH_NOT_CONFIGURED`, and it accepts the token only as `Authorization: Bearer` or `X-Dashboard-Token`, never `?token=`, which would land in proxy access logs. Token matching is constant-time. |
...
| `server/lib/data-transfer.js` | Full-dataset export/import ("backup / restore"). `buildExportBundle(db, stmts)` serializes every user-owned table (sessions, agents, events, token_usage, workflows, dashboard_runs, alert_rules, model_pricing, cursor_model_pricing, gpt_model_pricing) into one versioned JSON bundle (`format: "ccam-export"`); machine-bound/secret tables (push_subscriptions, webhook_targets/deliveries, alert_events, local Codex/Cursor ingest cursors) are excluded. `importExportBundle(db, bundle)` restores it session-atomically inside one transaction with `defer_foreign_keys` ON: a session already present (by UUID) is skipped WHOLE (with its agents/events/token_usage/workflows) so re-import and cross-machine merges never duplicate or clobber; events are re-inserted without their non-portable autoincrement id; config rows use `INSERT OR IGNORE` on their natural key. Backs `GET/POST /api/settings/{export,import}` and `ccam import-data` |
...
### Making a keyboard-only feature discoverable

A launcher nobody knows about is a launcher nobody opens, and the palette has no
permanent affordance by design — a button beside the sidebar duplicates
navigation the sidebar already shows. The answer is not standing chrome for a
fact you learn in one second, but a hint that appears where the need is felt and
**deletes itself once it has done its job**.

`client/src/lib/paletteDiscovery.ts` holds one bit in `localStorage`: has this
browser ever opened the palette? Two surfaces read it:

| Surface | Why there |
| --- | --- |
| The splash screen's closing line | First run is the one moment every user passes through, and the splash already shows exactly once per browser |
| A `⌘K` chip inside the Sessions and Agent Config search fields (`PaletteHint`) | The user is already searching in a narrow field; the chip says a broader one exists. It decorates an input that exists rather than adding chrome |

Opening the palette flips the bit and both disappear, permanently and without a
reload — the store notifies through `useSyncExternalStore`. The chip is
`aria-hidden` and `pointer-events-none`: it annotates a text field, so announcing
it or catching a click aimed at the input would both be worse than saying
nothing, and screen-reader users get the same fact as readable prose on the
splash. Inside the Sessions field it hides below `lg`, where it would otherwise
clip the placeholder to "Search ses" — a truncated label is a worse trade than a
hint shown less often.
...
| `idx_events_tool_name` | events | `tool_name` **WHERE** `tool_name IS NOT NULL` | The analytics tool-usage panel groups every event by `tool_name`. Unindexed that is a full events scan on every request, and `better-sqlite3` is synchronous, so the whole server stalls for its duration — measured at 45.7 s on a 3.9M-row table, 0.25 s with this index. Only tool events carry a `tool_name` (157k of those 3.9M rows), so the partial predicate keeps the index a small fraction of the table |
| `idx_events_session_type_uuid` | events | `session_id, event_type, json_extract(data,'$.uuid')` **WHERE** `json_valid(data) = 1 AND event_type IN ('RemoteToolEvent','RemoteTurn')` | Serves the `POST /api/hooks/ingest-batch` dedup, which would otherwise re-evaluate `json_extract()` over every row of the session for every batch item. Partial on two counts: `json_valid(data)` because legacy rows predating the JSON-events convention would make `json_extract()` abort startup at index-build time, and the `event_type` list because the index otherwise costs every installation (~28 MB per 500k events, ~224 MB on a 4M-row database) whether or not remote push is configured. SQLite only uses a partial index when the query's own `WHERE` provably implies the index's, so the dedup query repeats **both** predicates literally — a bare `event_type = ?` parameter is not provably inside the `IN` list to the planner and falls back to a full scan. An older, broader copy of this index is dropped and rebuilt on startup so an existing install picks up the narrowing |
...
4. agents
5. events and hooks
6. Claude/Cursor/GPT pricing and cost
7. workflows
8. alerts
9. webhooks
...
graph LR
    ...
    DEV --> SERVER["scripts/dev-server.js<br/>Port 4820<br/>Debounced graceful restart"]
...
| **File watching** | Debounced server watcher + Vite HMR | None                            |
