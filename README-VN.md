...
Cursor không cần lựa chọn thiết lập riêng: chọn **Claude Code** trên màn hình chào cũng bật giám sát Cursor ngay. Trình theo dõi hệ thống tệp phát hiện `~/.cursor/chats/*/<session>/meta.json` ngay khi `agent` khởi động, trước khi có transcript; thay đổi trong `prompt_history.json` cập nhật thẻ phiên, trạng thái đang làm việc và Conversation ngay khi gửi prompt. Khi `~/.cursor/projects/*/agent-transcripts` xuất hiện, Dashboard hợp nhất lượt người dùng đang chờ mà không lặp, bổ sung tiêu đề/dự án/lượt/agent phụ và snapshot JSONL chính cùng agent phụ.

Thẻ Cursor có tiêu đề `Cursor · <tên>` và phụ đề dự án/lượt/agent phụ. Mục **Giá Cursor** riêng trong Cài đặt bao gồm Grok, Composer và danh mục mô hình bên thứ ba đã công bố, không dùng nhầm giá Claude hay Codex. Có thể đổi thư mục qua `DASHBOARD_CURSOR_HOME` và điều chỉnh polling dự phòng qua `DASHBOARD_CURSOR_SYNC_MS`; `0` chỉ tắt quét định kỳ, còn theo dõi hệ thống tệp trực tiếp vẫn bật.
...
| `POST` | `/api/hooks/ingest-batch` | Đẩy một batch dữ liệu phiên từ máy roaming/sau NAT (mặc định tắt; xem `REMOTE_PUSH_TOKEN` bên dưới và `server/README.md` để biết cấu trúc payload đầy đủ) |
...
| `RemoteToolEvent` | Tool call do một máy từ xa đẩy lên | Được `POST /api/hooks/ingest-batch` ghi cho mỗi mục `tool_events[]` mà một máy roaming/sau NAT đẩy lên. Khử trùng lặp theo `(session_id, event_type, uuid)` với cả hàng đã commit lẫn trong cùng batch, nên gửi lại batch là an toàn |
| `RemoteTurn` | Thời lượng lượt do một máy từ xa đẩy lên | Được `POST /api/hooks/ingest-batch` ghi cho mỗi mục `turns[]`, mang theo `duration_ms` của lượt đó. Khử trùng lặp giống `RemoteToolEvent` |
