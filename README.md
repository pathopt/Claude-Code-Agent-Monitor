...
Cursor needs no separate setup choice: selecting **Claude Code** on the splash screen also enables Cursor monitoring. A filesystem watcher discovers `~/.cursor/chats/*/<session>/meta.json` as soon as `agent` starts, before Cursor creates a transcript; changes to `prompt_history.json` update the session card, working state, and Conversation immediately when a prompt is submitted. When `~/.cursor/projects/*/agent-transcripts` arrives, the dashboard merges it into the pending conversation without duplicating the user turn, enriches native titles/projects/turns/subagents, and snapshots main and subagent JSONL so Conversation remains available after Cursor cleans its own history.

Cursor cards receive native `Cursor · <title>` headings and project/turn/subagent subtitles. Costs use a dedicated, editable **Cursor Pricing** table in Settings—covering Cursor Grok 4.6/4.5, Composer 2.5, and the published third-party model catalog—rather than borrowing Claude or Codex rates. Override the source root with `DASHBOARD_CURSOR_HOME`; tune the polling fallback with `DASHBOARD_CURSOR_SYNC_MS` (`0` disables only periodic scanning; live filesystem watching stays enabled).
...
| `npm run repair-tokens` | Re-derive **non-workflow** token totals for every **Claude** session with a transcript on disk (found under `~/.claude/projects/` or via the session's stored `transcript_path`) and zero the compaction baselines; workflow and Codex rows are preserved. One-time repair for databases inflated by the pre-v2.0.9 per-record usage sum. Stop the dashboard first |
| `DASHBOARD_TOKEN_REPAIR` | `1` (enabled) | One-time automatic repair of token totals inflated before usage was reconciled per `message.id`. `replaceTokenUsage` is a high-water mark, so the parser fix alone can never lower a historical total — without this pass every pre-upgrade session keeps its inflated cost forever. Runs once per database (marker-gated, deferred off the boot path, skipped while another dashboard shares the data directory) and snapshots the old rows to `token_usage_pre_repair` first. Set to `0` to skip it and repair manually with `npm run repair-tokens` |
